...
| [jDMRgrid](https://github.com/robert-piecyk/jDMRgrid) | Heuristic caller for differentially methylated regions in large-scale WGBS data. The genome is split into sliding windows of user-defined size, methylation states are called per window with a hidden Markov model, then samples are merged and non-polymorphic patterns filtered. Built for population-level studies and control/treatment designs. | R |
